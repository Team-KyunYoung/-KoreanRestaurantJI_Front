...
const Cart = () => {
  ...
  function onChangeCheck(cartNumber, dishNumber, cartQuantity, event) {
    ...
    if (event.target.checked) {
      setDishOrderList((dishOrderList) => [
        ...dishOrderList,
    ...
    dishQuantity.map((quantityData) =>
      quantityData.cartNumber === cartNumber
        ? ((quantity = quantityData.quantity),
          (cartPrice = quantityData.cartPrice))
        : null
    );
  ...
  function handleChangeQuantity(
    ...
    CartService.updateCartQuantitiy(cartNumber, cartQuantity)
      .then((response) => {
          ...
          dishQuantity.map((data) =>
            data.cartNumber === cartNumber
              ? {
                  ...
                  cartPrice: cartQuantity * dishPrice,
                }
              : data
          )
        );
        ...
        if (isPlus) setOrderPrice((current) => current + dishPrice);
        else setOrderPrice((current) => current - dishPrice);
      })
      .catch((error) => {
        console.log(error.response);
      });
    ...
    return (
      <div className={styles.cartDish} id={styles.checkbox}>
        ...
        <div className={styles.cartDetail}>
          ...
          <div className={styles.cartQuantity}>
            <button
              className={`${styles.countbutton} ${
                cartQuantity === 1 ? styles.disableclick : null
              }`}
              id="minusBtn"
              onClick={() =>
            ...
            <button
              className={`${styles.countbutton} ${styles.plus}`}
              onClick={() =>
                handleChangeQuantity(
                  cartNumber,
                  dishNumber,
          ...
          <div className={styles.cartDishPrice}>
            {cartPrice.toLocaleString("ko-KR")} 원
          </div>
        </div>
      </div>
    );
      ...
      <main className={styles.container}>
        ...
        <div className={styles.mainContents}>
            ...
            {isLoading ? (
              ...
              cartList.map((cartdish) => (
                <CartDish
                  ...
                  cartQuantity={
                    cartQuantityPriceCheck(cartdish.cartNumber).quantity
                  }
                  cartPrice={
                    cartQuantityPriceCheck(cartdish.cartNumber).cartPrice
                  }
                />
              ))
            )}
          ...
          <div className={styles.orderContents}>
            <div>
              <span id={styles.total}>Total</span>{" "}
              <span id={styles.totalPrice}>
                {orderPrice.toLocaleString("ko-KR")} 원
              </span>
            </div>
            <div>
              <button id={styles.orderButton} onClick={handleOrderClick}>
                주문하기
              </button>
            </div>
          </div>
      ...
      >
        ...
        <div className={modalstyles.toGoBtns}>
          <button className={modalstyles.submit}>
            <Link to="/UserInfo/ordered/blank">주문 확인하기</Link>
          </button>
          <button
